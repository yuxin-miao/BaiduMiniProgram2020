...
const COMPUTED_STYLE = ['color', 'font-size', 'font-weight', 'font-family', 'backgroundColor', 'border', 'border-radius', 'box-sizing', 'line-height'];
const DEFAULT_BORDER = '0px none rgb(0, 0, 0)';
const DEFAULT_BORDER_RADIUS = '0px'; // default z-index??
...
const drawText = (context, data) => {
  ...
  context.font = `${fontWeight} ${Math.round(parseFloat(fontSize))}px ${fontFamily}`;
...
const getImgInfo = src => new Promise((resolve, reject) => {
  swan.getImageInfo({
    src,
...
const getCalculatedImagePosition = (imgData, naturalWidth, naturalHeight) => {
  ...
  const {
    ...
    imgLeft,
    imgTop
  } = getImgRect(imgData, borderWidth);
  const ratio = naturalWidth / naturalHeight; // tweak for real width and position => center center
...
const drawArcImage = (context, imgData) => {
  ...
  return getImgInfo(src).then(res => {
    ...
    context.drawImage(src, 0, 0, naturalWidth, naturalHeight, left, top, realWidth, realHeight);
    context.restore();
  });
};
...
const drawRectImage = (context, imgData) => {
  ...
  return getImgInfo(src).then(res => {
    ...
    context.drawImage(src, 0, 0, naturalWidth, naturalHeight, realLeft, realTop, realWidth, realHeight);
    context.restore();
  });
};
...
const drawRectBorder = (context, imgData) => {
  ...
  context.setStrokeStyle(borderColor);
  context.rect(left + borderWidth / 2, top + borderWidth / 2, width - borderWidth, height - borderWidth);
  context.stroke();
  context.restore();
...
const drawViewArcBorder = (context, imgData) => {
  ...
  const {
    borderWidth,
    borderColor
  } = getBorderAttributes(border); // console.log('🐞-imgData', imgData)

  context.beginPath();
  context.moveTo(left + borderRadius, top);
  context.lineTo(left + width - borderRadius, top);
  context.arcTo(left + width, top, left + width, top + borderRadius, borderRadius);
  context.lineTo(left + width, top + height - borderRadius);
  context.arcTo(left + width, top + height, left + width - borderRadius, top + height, borderRadius);
  context.lineTo(left + borderRadius, top + height);
  context.arcTo(left, top + height, left, top + height - borderRadius, borderRadius);
...
const drawViewBezierBorder = (context, imgData) => {
  ...
  context.quadraticCurveTo(left + width, top + height, left + width - borderRadiusX, top + height);
  context.lineTo(left + borderRadiusX, top + height);
  context.quadraticCurveTo(left, top + height, left, top + height - borderRadiusY);
...
const formatElementData = elements => elements.map(element => {
...
const getSortedElementsData = elements => elements.sort((a, b) => {
  if (a.rank < b.rank) {
    return -1;
...
const drawElementBaseOnIndex = (context, storeObject, key = 0, drawPromise) => {
  ...
  const chainPromise = drawPromise.then(() => {
    const nextPromise = storeObject[objectKey] ? Promise.all(drawElements(context, storeObject[objectKey])) : Promise.resolve();
    return nextPromise;
  });
...
const drawCanvas = (canvasId, wrapperData, innerData) => {
  ...
  context.setTextBaseline('top'); // todo: use this after weixin fix stupid clip can't work bug in fillRect
  ...
  return drawElementBaseOnIndex(context, storeObject).then(() => new Promise((resolve, reject) => {
    context.draw(true, () => {
      resolve();
    });
  }));
...
const wxSelectorQuery = element => new Promise((resolve, reject) => {
  try {
    swan.createSelectorQuery().selectAll(element).fields({
      dataset: true,
      size: true,
      rect: true,
